...
class EmailValidator:
    ...
    def __init__(self, sender_email: str = "verify@yourdomain.com", use_third_party: bool = True, third_party_provider: str = "rapid"):
        """
    ...
    def validate_email(self, email: str) -> Dict[str, any]:
        ...
        result = {
            ...
            'is_big_provider': False   # Gmail, Outlook, Yahoo, etc.
        }
        ...
        try:
            ...
            self.logger.debug(f"Email format valid: {normalized_email}")
        except EmailNotValidError as e:
            result['reason'] = 'invalid_format'
            result['smtp_response'] = str(e)
            result['confidence'] = 'high'  # Format definitely invalid
            self.logger.debug(f"Email format invalid: {email} | reason: {str(e)}")
            return result

        # Check if big provider (Gmail, Outlook, Yahoo, etc.)
        domain = normalized_email.split('@')[1].lower()
        big_providers = [
            'gmail.com', 'googlemail.com', 'outlook.com', 'hotmail.com',
            'yahoo.com', 'yahoo.co.uk', 'yahoo.co.jp', 'icloud.com',
            'me.com', 'aol.com', 'protonmail.com', 'mail.com'
        ]
        result['is_big_provider'] = domain in big_providers

        # Check for placeholder/invalid domains
        placeholder_domains = [
            ...
            'sentry.io', 'sentry.com', 'github.io', 'herokuapp.com',
            'dummy.com', 'fake.com', 'invalid.com', 'placeholder.com',
            'xxx.com', 'noreply.com', 'no-reply.com'
        ]

        # Check for generic/template patterns
        placeholder_patterns = [
            'example', 'test', 'sample', 'demo', 'dummy', 'fake',
            'placeholder', 'domain', 'website', 'email',
            'yourname', 'yourcompany', 'yourdomain', 'yoursite',
            'companyname', 'mycompany', 'myemail'
        ]

        is_placeholder = (
            domain in placeholder_domains or
            any(pattern in domain for pattern in placeholder_patterns) or
        ...
        if is_placeholder:
            result['valid'] = False
            result['reason'] = 'placeholder_domain'
            result['confidence'] = 'high'  # Definitely not real
            result['smtp_response'] = f'Placeholder/template domain detected: {domain}'
            self.logger.debug(f"Placeholder domain detected: {email} | domain: {domain}")
            return result

        # Try third-party validation first if enabled
        if self.use_third_party:
            try:
                self.logger.debug(f"Attempting third-party validation: {normalized_email}")
                tp_result = self._verify_with_rapid(normalized_email)
                if tp_result and tp_result.get('reason') not in ('third_party_error', 'smtp_error'):
                    self.logger.debug(f"Third-party validation: {normalized_email} | valid: {tp_result.get('valid')} | reason: {tp_result.get('reason')}")
                    return tp_result
                else:
                    self.logger.debug(f"Third-party validation failed, falling back to SMTP: {normalized_email}")
            except Exception as e:
                self.logger.debug(f"Third-party validation error: {normalized_email} | error: {str(e)}")
                pass  # Continue to local validation
        
        try:
            # Use validate_email for comprehensive validation
            self.logger.debug(f"Starting SMTP validation: {normalized_email}")
            ...
            if validation_result is True:
                ...
                if catch_all_result:
                    ...
                    result['confidence'] = 'low'  # Catch-all = uncertain
                    self.logger.debug(f"SMTP valid but catch-all: {normalized_email}")
                elif result['is_big_provider']:
                    result['confidence'] = 'low'  # Big providers = false positive risk
                    self.logger.debug(f"SMTP valid but big provider: {normalized_email}")
                else:
                    result['confidence'] = 'high'  # Small domain + SMTP valid = likely real
                    self.logger.debug(f"SMTP valid with high confidence: {normalized_email}")

            elif validation_result is False:
                result['reason'] = 'smtp_invalid'
                result['confidence'] = 'high'  # Definitely invalid
                self.logger.debug(f"SMTP invalid: {normalized_email}")
                ...
                self.logger.debug(f"SMTP ambiguous: {normalized_email}")
            ...
            result['smtp_response'] = str(e)
            self.logger.debug(f"SMTP error: {normalized_email} | reason: {result['reason']} | error: {str(e)}")
